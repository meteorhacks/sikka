...
Firewall._updateStats = function _updateStats(ip, sessionId, humanToken) {
  console.log("_updateStats", ip, sessionId, humanToken);
  ...
  var blocked = false;
  if(Firewall._rateExceeds("perIp", ip, 50)) {
    console.log("IP Blocked.", ip);
    Firewall._blockIpFor(ip, 1000 * 60 * 2);
    console.log("isBlocked", Firewall._isBlocked(ip));
    blocked = true;
  }

  if(Firewall._isValidHuman(humanToken)) {
    if(Firewall._rateExceeds("perHuman", humanToken, 10)) {
      console.log("Human Deleted.", humanToken);
...
Firewall._rateExceeds = function(type, key, maxValue) {
  ...
  console.log("***RATE", rate, timeDiffSecs);
